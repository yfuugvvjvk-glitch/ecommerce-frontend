...
export default function ChangeEmailModal({ isOpen, onClose, currentEmail, onSuccess }: ChangeEmailModalProps) {
  ...
  const handleSendCode = async (e: React.FormEvent) => {
    ...
    if (newEmail === currentEmail) {
      setError('Noul email nu poate fi identic cu cel curent.');
      return;
    }
    ...
    try {
      ...
      await apiClient.post('/api/user/change-email', { newEmail });
  ...
  const handleVerifyCode = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await apiClient.post('/api/user/verify-email-change', {
        code: verificationCode,
      });

      if (response.data.success) {
        alert('Email schimbat cu succes!');
        onSuccess();
        onClose();
      }
    } catch (err: any) {
      ...
      setCode(['', '', '', '', '', '']);
      const firstInput = document.getElementById('email-code-0');
      firstInput?.focus();
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold">
            {step === 'input' ? 'Schimbă Email' : 'Verificare Email'}
          </h2>
          <button
            onClick={onClose}
        ...
        {step === 'input' ? (
          <form onSubmit={handleSendCode} className="space-y-4">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-md p-3 text-sm text-blue-800">
              ...
              <p>Veți primi un cod de verificare la noul email și o notificare la email-ul curent.</p>
            </div>
          ...
          <form onSubmit={handleVerifyCode} className="space-y-4">
            <div className="text-center mb-4">
              ...
              <p className="text-blue-600 font-semibold mt-1">{newEmail}</p>
            </div>
